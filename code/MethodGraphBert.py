...
class MethodGraphBert(BertPreTrainedModel):
    ...
    def forward(self, x, d, w, wl, head_mask=None, residual_h=None, context_stop_grad_idx=None):
        if head_mask is None:
            head_mask = [None] * self.config.num_hidden_layers

        embedding_output = self.embeddings(x, d, w, wl)
        encoder_outputs = self.encoder(embedding_output, head_mask=head_mask, residual_h=residual_h, context_stop_grad_idx=context_stop_grad_idx)
